...
class DQNAgent:
    def __init__(self, env, hidden_dim=128, gamma=0.99, lr=1e-3, batch_size=32, buffer_capacity=10000):
        self.env = env
        self.gamma = gamma
    ...
    def select_action_1(self, state):
        ...
        state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).unsqueeze(-1)  # Add batch dimension
        with torch.no_grad():
